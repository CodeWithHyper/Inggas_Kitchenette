      ...
      .then((reviews) => {
        ...
        function displayReviews(count) {
            ...
            for (let i = 0; i < count && i < reviews.length; i++) {
                const card = createReviewCard(reviews[i].name, reviews[i].rating, reviews[i].review_text);
...
loadReviews();
  ...
  window.submitReview = function(event) {
    ...
    fetch(`verify_contact.php?email=${encodeURIComponent(email)}`)
        ...
        .then(data => {
            if (!data.exists) {
                alert("Your email is not registered in our contacts. You cannot post a review.");
                return;
            }
           ...
           fetch("save_review.php", {
             ...
             body: `name=${encodeURIComponent(name)}&email=${encodeURIComponent(email)}&rating=${encodeURIComponent(rating)}&reviewtext=${encodeURIComponent(text)}`
            })
